...
drop policy if exists "anonymous users insert" on public.users;
create policy "anonymous users insert" on public.users for insert to anon, authenticated with check (role = 'user' and is_blocked = false);
drop policy if exists "anonymous users read" on public.users;
create policy "anonymous users read" on public.users for select to anon, authenticated using (is_blocked = false);
